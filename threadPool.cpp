...
std::unordered_set<std::thread::id> ss;
...
class ThreadPool
{
        ...
        for (size_t i{}; i < nThreads; ++i) {
            workerThreads.emplace_back([this] {
                while (true) {
                    ...
                    ss.insert(std::this_thread::get_id());
    ...
    auto submit(F&& f, Args&&... args) -> std::future<std::invoke_result_t<F, Args...>>
    {
...

        {
            ...
            if (stop.load()) {
                throw std::runtime_error("Cannot submit a task to a stopped thread pool.");
            }
...
int main()
{
    ...
    std::cout << "Total unique threads used: " << ss.size() << std::endl;
    for (auto i : ss) std::cout << i << " ";
    std::cout << std::endl;

    auto end = std::chrono::steady_clock::now();

    // Calculate and print total execution time
    auto elapsed_ms = std::chrono::duration_cast<std::chrono::seconds>(end - start).count();
    std::cout << "Total time: " << elapsed_ms << " seconds\n";
}
